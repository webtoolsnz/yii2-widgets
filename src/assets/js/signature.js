(function (window, $) {
  ...
  $.fn.signatureWidget = function (options) {
    return this.each(function () {
      ...
      var data = input.val();
      var toggleContent =
        '<span class="sig-placeholder">Click here to sign</span>';
      ...
      options = $.extend(
        {
          ...
          modal: {
            title: "Sign Below",
            buttons: [
              '<button type="button" class="btn btn-secondary sig-clear">Clear</button>',
      ...
      var tpl = [
        '<a class="sig-toggle" id="' +
          ts +
          '_toggle" href="#" data-toggle="modal" data-target="#' +
          ts +
          '_modal">',
        data !== "" && data.length > 0
          ? '<img src="' + data + '">'
